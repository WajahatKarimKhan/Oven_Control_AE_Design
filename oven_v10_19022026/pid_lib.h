...
class QuickPID {
  public:
    QuickPID(double* input, double* output, double* setpoint, double kp, double ki, double kd, int controllerDirection, int RodType);
    
    // Configuration
    void SetMode(int mode); // AUTOMATIC = 1, MANUAL = 0
    void SetOutputLimits(double min, double max);
    void SetTunings(double kp, double ki, double kd, int RodType);
    void SetPOn(int pOn);
