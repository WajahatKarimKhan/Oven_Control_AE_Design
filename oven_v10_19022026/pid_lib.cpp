...
QuickPID::QuickPID(double* input, double* output, double* setpoint, double kp, double ki, double kd, int controllerDirection, int RodType) {
  myOutput = output;
  myInput = input;
  mySetpoint = setpoint;
  inAuto = false;
  ...
  QuickPID::SetTunings(kp, ki, kd, RodType);
...
void QuickPID::SetTunings(double Kp, double Ki, double Kd, int RodType) {
  log_Paramters[RodType][5] = 1;
