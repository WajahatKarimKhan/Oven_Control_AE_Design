...
bool isStatusUpdateDue() {
  ...
  if (now - lastStatusUpdateTime >= statusUpdateInterval) {
    lastStatusUpdateTime = now;
    Log_funcTime[3] = millis() - now;
    return true;
  }
  Log_funcTime[3] = millis() - now;
  return false;
...
void processIncomingStream(Stream& port) {
  ...
  // --- Not Used yet: SET INDIVIDUAL PID COMMAND ---
  ...
  if (strcmp(target, "rod1") == 0) {
      ...
      pidRod1.SetTunings(kp, ki, kd, TOPROD);
      updated = true;
    } else if (strcmp(target, "rod2") == 0) {
      settings.rod2Pid = params;
      pidRod2.SetTunings(kp, ki, kd, BOTTOMROD);
      updated = true;
    } else if (strcmp(target, "steam") == 0) {
      settings.rodSteamPid = params;
      pidSteam.SetTunings(kp, ki, kd, STEAMROD);
      updated = true;
    }
...
void printDebugInfo() {
  ...
  Serial.println(pidSetpointRod1);
